...
class Turn:
    ...
    board: Board
    black_to_move: int = 1
    metadata: tuple = field(default=(), compare=False, hash=False)
...
class Transition:
    ...
    def __post_init__(self):
        rule_indices = tuple(int(index) for index in self.rule_indices)
        if not rule_indices:
...
class Ruleset:
    """An immutable collection of primitive rules with native Python views."""
    ...
    def __init__(self, rules):
        ...
        self._records = tuple(_indexed_record(index, rule) for index, rule in enumerate(rule_tuple))
        self._rule_set = frozenset(rule.as_tuple for rule in rule_tuple)
        self._condition_tuples = tuple(rule.conditions.as_tuple for rule in rule_tuple)
        self._effect_tuples = tuple(rule.effects.as_tuple for rule in rule_tuple)
    ...
    def record_map(self):
        ...
        return {index: dict(record) for index, record in enumerate(self._records)}

    @property
    def rule_set(self):
        """Return an independent set of hashable primitive-rule tuples."""

        return set(self._rule_set)

    @property
    def condition_tuples(self):
        """Return the immutable condition tuples."""

        return self._condition_tuples

    @property
    def effect_tuples(self):
        """Return the immutable effect tuples."""

        return self._effect_tuples

    @property
    def rules_by_side(self):
        """Return an independent side-to-rule-index mapping."""

        return dict(self._rules_by_side)

    @property
    def rules_by_metadata(self):
    ...
    def __getitem__(self, rule_index):
        return self._rules[self._check_index(rule_index)]

    def record(self, rule_index):
        """Return one rule as a plain dictionary."""

        return dict(self._records[self._check_index(rule_index)])
    ...
    def to_dataframe(self):
        ...
        df = pd.DataFrame.from_records(self._records, columns=("rule_index", *RULE_COLUMNS))
    ...
    def indices_for(self, black_to_move=None, king=None, promotion=None, capture=None):
        ...
        if all(value is not None for value in (black_to_move, king, promotion, capture)):
            ...
            return list(self._rules_by_metadata.get(key, ()))

        indexes = range(len(self))
        return [
            index for index in indexes if _metadata_matches(self._rules[index], black_to_move, king, promotion, capture)
        ]
    ...
    def matching_rule_indices(self, turn, captures_only=False, from_mask=None):
        ...
        turn = as_turn(turn)
        indexes = self._rules_by_side[turn.black_to_move]
        if captures_only:
            indexes = [index for index in indexes if self._rules[index].effects.capture]
        if from_mask is not None:
            from_mask = _mask32(from_mask, "from_mask")
            indexes = [index for index in indexes if self._rules[index].conditions.mover == from_mask]
        return [index for index in indexes if self._rules[index].applies(turn)]

    def matching_rules(self, turn, captures_only=False, from_mask=None):
        """Return matching rule objects."""

        return tuple(self._rules[index] for index in self.matching_rule_indices(turn, captures_only, from_mask))

    def legal_rule_indices(self, turn, from_mask=None):
        """Return legal primitive rule indexes, applying mandatory capture."""

        matches = self.matching_rule_indices(turn)
        captures = [index for index in matches if self._rules[index].effects.capture]
        legal = captures if captures else matches
        if from_mask is None:
            return legal
        from_mask = _mask32(from_mask, "from_mask")
        return [index for index in legal if self._rules[index].conditions.mover == from_mask]

    def legal_rules(self, turn, from_mask=None):
        """Return legal primitive rule objects."""

        return tuple(self._rules[index] for index in self.legal_rule_indices(turn, from_mask=from_mask))

    def legal_moves(self, turn, from_mask=None):
        """Return legal first-step primitive moves as plain 32-bit-mask dictionaries."""
    ...
    def apply_rule(self, turn, rule_index, switch_side=True):
        ...
        return self._rules[self._check_index(rule_index)].apply(turn, switch_side=switch_side)

    def legal_transitions(self, turn):
        ...
        if not indexes:
            return ()
        if not self._rules[indexes[0]].effects.capture:
        ...
        for index in indexes:
            ...
            if rule.effects.promotion:
                transitions.append(Transition((index,), _finish_turn(primitive_turn)))
            else:
                transitions.extend(self._capture_transitions(primitive_turn, rule.mover_effect, (index,)))
    ...
    def _capture_transitions(self, turn, from_mask, path):
        indexes = self.matching_rule_indices(turn, captures_only=True, from_mask=from_mask)
        if not indexes:
        ...
        for index in indexes:
            ...
            if rule.effects.promotion:
                ...
                transitions.extend(self._capture_transitions(primitive_turn, rule.mover_effect, next_path))
        return transitions
...
def _finish_turn(turn):
    return Turn(turn.board, 1 - turn.black_to_move, turn.metadata)
