...
class PackageApiTests(unittest.TestCase):
    ...
    def test_primitive_rule_runtime_catalog_lists_preconditions_and_effects(self):
        ...
        runtime = ruleset.to_dataframe()

        self.assertIs(ruleset, american_ruleset())
        self.assertEqual(ruleset.is_black.typecode, "I")
        self.assertEqual(ruleset.is_white.typecode, "I")
        self.assertEqual(ruleset.is_empty.typecode, "I")
        self.assertEqual(ruleset.be_black.typecode, "I")
        self.assertEqual(ruleset.be_white.typecode, "I")
        self.assertEqual(ruleset.be_empty.typecode, "I")
        self.assertEqual(ruleset.flags.typecode, "B")
        self.assertEqual(len(runtime), 510)
        self.assertEqual(len(primitive_rule_runtime_catalog_df()), 510)
        ...
        mask_columns = ["is_black", "is_white", "is_empty", "be_black", "be_white", "be_empty"]
        self.assertTrue((runtime[mask_columns] <= MASK32).all().all())
        ...
        self.assertEqual(len(ruleset.indices_for(black_to_move=1)), 255)
        self.assertEqual(len(ruleset.indices_for(capture=1)), 216)
        self.assertEqual(len(ruleset.indices_for(promotion=1)), 26)
        self.assertEqual(len(ruleset.indices_for(king=1)), 340)
        self.assertFalse(
            {
                "after_black",
                "after_white",
        ...
        self.assertEqual(first["is_black"], mask64_to32(square_mask(0, 1)))
        self.assertEqual(first["is_white"], 0)
        self.assertEqual(first["is_empty"], mask64_to32(square_mask(1, 0)))
        self.assertEqual(first["be_black"], mask64_to32(square_mask(1, 0)))
    ...
    def test_ruleset_selection_and_turn_state_edges(self):
        ruleset = Ruleset.american()
        state = BoardState.initial()
        turn_state = TurnState.from_board_state(state)
        first_index = ruleset.legal_rule_indices(turn_state)[0]
        first_record = ruleset.record(first_index)

        self.assertEqual(turn_state.as_tuple(), as_turn_state(state.as_tuple()).as_tuple())
        self.assertEqual(TurnState.from_tuple(turn_state.as_tuple()), turn_state)
        self.assertEqual(TurnState(0, 0, 0, "W").side, "W")
        self.assertEqual(square32_to64(square64_to32(square_mask(0, 1))), square_mask(0, 1))
        self.assertIsNone(square64_to32(0))
        with self.assertRaises(ValueError):
            square64_to32(square_mask(0, 0))
        with self.assertRaises(ValueError):
            square32_to64(32)
        with self.assertRaises(ValueError):
            TurnState(1, 1, 0, 1)
        ...
        self.assertEqual(ruleset.select_records(0), [ruleset.record(0)])
        self.assertEqual(len(ruleset.select_records(slice(0, 2))), 2)
        self.assertEqual(ruleset.select_records([0, 1]), [ruleset.record(0), ruleset.record(1)])
        self.assertEqual(ruleset.select_records([first_record]), [first_record])
        self.assertGreater(len(ruleset.indices_for(black_to_move=1, king=0, promotion=0, capture=0)), 0)
        with self.assertRaises(IndexError):
            ruleset.record(-1)
        with self.assertRaises(ValueError):
            Ruleset([1], [0, 0], [1], [1], [0], [1], [0])
        with self.assertRaises(ValueError):
            Ruleset.american("red")

        dataframe_ruleset = Ruleset.from_records(ruleset.to_dataframe().iloc[:2])
        series_ruleset = Ruleset.from_records(ruleset.to_dataframe().iloc[0])
        dict_ruleset = Ruleset.from_records(first_record)
        copied_ruleset = Ruleset.from_records(dataframe_ruleset)
        self.assertEqual(len(dataframe_ruleset), 2)
        self.assertEqual(len(series_ruleset), 1)
        self.assertEqual(len(dict_ruleset), 1)
        self.assertEqual(len(copied_ruleset), 2)

        white_rule = ruleset.indices_for(black_to_move=0)[0]
        self.assertFalse(ruleset.rule_applies(turn_state, white_rule))
        move = ruleset.rule_move(first_index)
        blocked_state = TurnState(
            turn_state.black | mask64_to32(move["to_mask"]),
            turn_state.white,
            turn_state.kings,
            turn_state.black_to_move,
        )
        self.assertFalse(ruleset.rule_applies(blocked_state, first_index))
        with self.assertRaises(ValueError):
            ruleset.apply_rule(blocked_state, first_index)
        self.assertEqual(
            ruleset.apply_rule(turn_state, first_index, switch_side=False).black_to_move,
            turn_state.black_to_move,
        )
    ...
    def test_primitive_rules_apply_with_bitmask_conditions(self):
        ...
        matching_indexes = ruleset.legal_rule_indices(state)
        legal = state.legal_moves()

        self.assertEqual(len(matching_rules), len(legal))
        self.assertEqual(len(matching_forced_rules), len(legal))
        self.assertEqual(len(matching_indexes), len(legal))
        self.assertEqual(len(ruleset.successors(state)), len(legal))
        self.assertEqual(
        ...
        self.assertEqual(
            {(move["from_mask"], move["to_mask"], move["captured_mask"]) for move in ruleset.legal_moves(state)},
            {(move["from_mask"], move["to_mask"], move["captured_mask"]) for move in legal},
        )

        turn_state = TurnState.from_board_state(state)
        next_turn_state = ruleset.apply_rule(turn_state, matching_indexes[0])
        next_board_state = ruleset.apply_rule_to_board(state, matching_indexes[0])
        self.assertIsInstance(next_turn_state, TurnState)
        self.assertEqual(next_turn_state.as_board_state(), next_board_state)
        self.assertEqual(mask32_to64(next_turn_state.black), next_board_state.black)
        ...
        self.assertEqual(len(ruleset.legal_rule_indices(king_state)), 4)
        ...
        forced_indexes = ruleset.legal_rule_indices(capture_state)
        self.assertEqual(len(forced_rules), 1)
        self.assertEqual(len(forced_indexes), 1)
        self.assertTrue(forced_rules[0]["is_capture"])
        self.assertTrue(ruleset.record(forced_indexes[0])["capture"])
    ...
    def test_display_helpers_return_figures(self):
        ...
        runtime_rule_rows = ruleset.plot(rules=primitive_rule_runtime_catalog_df().iloc[:1], size=1, show=False)
        ...
        for rule_fig, rule_axes in [*rule_rows, *runtime_rule_rows, *indexed_rule_rows]:
        ...
        for rule_fig, _rule_axes in [*rule_rows, *runtime_rule_rows, *indexed_rule_rows]:
            plt.close(rule_fig)
